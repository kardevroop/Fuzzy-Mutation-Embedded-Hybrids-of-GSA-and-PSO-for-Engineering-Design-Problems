function [timeexc,gbest,pbest]=main_final_working(functionNum)
    tic
    %clc
    rng('shuffle');
%     ubArray=[100,10,100,100,30,100,1.28,500,5.12,32,600,50,50,65.53,5,5,15,5,1,1,10,10,10,60];
%     lbArray=[-100,-10,-100,-100,-30,-100,-1.28,-500,-5.12,-32,-600,-50,-50,-65.53,-5,-5,-5,-5,0,0,0,0,0,12];
%     dimArray=[30,30,30,30,30,30,30,30,30,30,30,30,30,2,4,2,2,2,3,6,4,4,4,4];
...
%    dimension=dimArray(functionNum);
    ...
    [population,ub,lb,dimension]=(datacreate_engg(functionNum,n,dimension));
    rank=(zeros(1,n));
...
%         population=max(population,lb);
    ...
    legend('Iteration vs Gbest','Iteration vs Average value');
    %}
    timeexc=toc;
end
        ...
        for j=1:size(population,2)
            if ((rand < pm))% || (pc > .98))
                ...
                if abs(population(i,j)) <= 0.00001
                    delta=.5*range(j)*((1-(count/iter))^2);
                else
                    delta=min(.5*range(j)*((1-(count/iter))^2),abs(population(i,j)));
                end
