function [per]=fitness(chromosome,ch) %all membership functions are defined here
    if (ch==1)
    ...
    elseif (ch==17)
        ...
        per=(chromosome(2)-(chromosome(1)^2)*5.1/(4*(pi^2))+5/pi*chromosome(1)-6)^2+10*(1-1/(8*pi))*cos(chromosome(1))+10;
    elseif (ch==18)
        x1=chromosome(1,1);
        x2=chromosome(1,2);
